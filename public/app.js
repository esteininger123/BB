...
// Anzeige schlicht als „Marktwert ImmoScout" bzw. „Marktwert Homeday" — dieses Label
// liefert nur den Portal-Namen. Alte Quellen-Werte ('schnitt'/'nur-is'/'nur-hd',
// z.B. aus gespeicherten Snapshots) werden mit gemappt.
  ...
  return new Promise((resolve) => {
    ...
    m.innerHTML =
      '<div class="reserv-modal fin-modal">' +
        '<h2>Kundenportal eröffnen</h2>' +
        '<div class="fin-hinweis fin-hinweis-allg">Der Kunde bekommt einen Portal-Link: Dort sieht er die <b>Objektunterlagen</b> zu seiner Wohnung. Läuft die <b>Finanzierung über uns</b>, lädt er im selben Portal seine Unterlagen hoch.</div>' +
        '<div class="reserv-modal-body">' +
          '<label class="fin-field fin-full"><span class="fin-label">Finanzierung über B&amp;B? <span class="nk-req">*</span></span>' +
            '<select id="fin-modus">' +
              '<option value="ueber_bb">Ja — läuft über uns</option>' +
              '<option value="offen" selected>Noch offen</option>' +
              '<option value="extern">Nein — Kunde finanziert extern</option>' +
            '</select></label>' +
          (snaps.length
            ? '<label class="fin-field fin-full"><span class="fin-label">Maßgebliche Kalkulation / Wohnung <span class="nk-req">*</span></span>' +
                '<select id="fin-snapshot">' + snapOptions + '</select>' +
                '<span class="fin-sub">Bestimmt, welche Objektunterlagen der Kunde sieht.</span></label>'
            : '<div class="nk-error">Kein Snapshot vorhanden — bitte zuerst eine Kalkulation speichern.</div>') +
          '<div id="fin-only" class="fin-only fin-full" hidden>' +
            '<div class="fin-hinweis">Vor dem Einreichen: Kalkulation als <b>Snapshot</b> gespeichert und <b>unterschriebene Selbstauskunft</b> liegt vor. Die Finanzierung kann erst mit unterschriebener Selbstauskunft einreichen.</div>' +
            '<div class="fin-grid">' +
        ...
        '<div class="reserv-modal-actions">' +
          ...
          '<button class="reserv-confirm" id="fin-save"' + (snaps.length ? '' : ' disabled') + '>Portal eröffnen</button>' +
        '</div>' +
      '</div>';
    const $ = (id) => m.querySelector('#' + id);
    const close = (val) => { m.remove(); document.removeEventListener('keydown', onKey); resolve(val); };
    ...
    // Modus steuert die Finanzierungs-Detailfelder: nur sichtbar, wenn über uns finanziert.
    const modusSel = $('fin-modus'), finOnly = $('fin-only');
    const applyModus = () => { finOnly.hidden = (modusSel.value !== 'ueber_bb'); };
    modusSel.addEventListener('change', applyModus);
    applyModus();
    const onSave = () => {
      const snapshotId = snaps.length ? $('fin-snapshot').value : '';
      if (!snapshotId) return;
      const modus = $('fin-modus').value;
      const ueberUns = modus === 'ueber_bb';
      const hb = ueberUns && $('fin-hb').checked, fb = ueberUns && $('fin-fb').checked;
      close({
        snapshotId,
        finanzierungUeberUns: modus,
        finanzierungsform: ueberUns ? $('fin-form').value : '',
        finanzierungsformAndere: ueberUns ? $('fin-form-andere').value.trim() : '',
        maxEigenkapital: (ueberUns && $('fin-maxek').value) ? Number($('fin-maxek').value) : null,
        notarterminZiel: ueberUns ? ($('fin-notar').value || '') : '',
        ...
        wasWichtig: ueberUns ? $('fin-wichtig').value.trim() : '',
        notizVertrieb: ueberUns ? $('fin-notiz').value.trim() : '',
      });
    };
...
function _finUebergabeEnsureStyles() {
  ...
  s.textContent = `
    ...
    .fin-hinweis-allg { background: #eef5f0; border-left-color: #2D6E47; color: #2c4a38; }
    .fin-sub { font-size: 0.78em; color: #8a8a8a; margin-top: 3px; }
    .fin-only { display: block; }
    .fin-only[hidden] { display: none; }
    .fin-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px 14px; margin-top: 6px; }
  `;
  document.head.appendChild(s);
}
...
async function uebergebeAnFinanzierung() {
  ...
  try {
    ...
    if (check && check.count > 0) {
      const msg = check.count === 1
        ? 'Für diesen Kunden wurde bereits ein Kundenportal eröffnet.'
        : `Für diesen Kunden wurden bereits ${check.count} Portale eröffnet.`;
      if (!confirm(msg + '\n\nWirklich ein WEITERES eröffnen?')) return;
    }
  } catch (e) { /* Check nicht kritisch — im Zweifel zulassen */ }
  ...
  try {
    const res = await api.post('/api/finanzierung/uebergeben', {
      ...
      finanzierungUeberUns: data.finanzierungUeberUns,
    ...
    try {
      ...
      const modusLabel = data.finanzierungUeberUns === 'ueber_bb'
        ? 'Finanzierung über uns'
        : (data.finanzierungUeberUns === 'extern' ? 'extern finanziert' : 'Finanzierung offen');
      await _appendActivityToNotizen('Kundenportal eröffnet' + (weBez ? ' — ' + weBez : '') + ' (' + modusLabel + ')');
    } catch (e) { /* Historie nicht kritisch */ }
    toast('Kundenportal eröffnet ✓', 'success');
...
function showUploadLinkModal(link) {
  ...
  m.innerHTML =
    '<div class="reserv-modal" style="max-width:480px">' +
      '<h2>Portal eröffnet ✓</h2>' +
      '<div class="reserv-modal-body">' +
        '<p style="margin:0 0 10px;font-size:0.92em;color:#444">Schick dem Kunden diesen Link — darüber sieht er seine Objektunterlagen und kann (falls Finanzierung über uns) seine Unterlagen hochladen:</p>' +
...
function renderTabUebersicht() {
  ...
  // Kundenportal eröffnen (2026-06-15, umgewidmet 2026-06-23) — bewusst hervorgehobene Aktions-Karte.
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap;">
        <div style="min-width:210px;">
          <div style="font-weight:800;font-size:15.5px;display:flex;align-items:center;gap:8px;margin-bottom:3px;">
            <span style="font-size:19px;">📂</span> Kundenportal eröffnen
          </div>
          <span class="text-tertiary text-small">Legt den Kundenordner an und erzeugt den Portal-Link: Objektunterlagen für den Kunden — plus Unterlagen-Upload, falls die Finanzierung über uns läuft.</span>
        </div>
        <button type="button" onclick="uebergebeAnFinanzierung()" title="Kundenportal eröffnen"
          ...
          onmouseover="this.style.background='#245a3a'" onmouseout="this.style.background='#2D6E47'">→ Eröffnen</button>
      </div>
    </div>`;
...
// Angezeigte Tag-0-Belastung = Summe der GERUNDETEN Positionen (Kosten je Bestandteil gerundet),
// identisch zur PDF-Logik (pdf.js belastungTag1Mo) — so zeigen App und PDF dieselbe Zahl (R5) und
// die sichtbaren Zeilen gehen exakt auf (R4). Fallback: gerundeter Roh-Wert (alte Snapshots).
function _belTag0Disp(r) {
  if (!r) return 0;
  if (r.mieteTag0Mo == null) return Math.round(_belTag0(r));
  return Math.round(r.mieteTag0Mo) + Math.round(r.stVorteilTag0Mo || 0)
    - Math.round(r.annuityMo || 0)
    - Math.round(r.hausgeldNurMo || 0) - Math.round(r.hausverwaltungMo || 0) - Math.round(r.mietverwaltungMo || 0);
}
...
function recalcAndRender() {
  ...
  const kpis = [
    ...
    kpiCard(_belTag0Disp(r) >= 0 ? 'Dein Überschuss / Monat' : 'Deine Belastung / Monat', fmtEurMo(_belTag0Disp(r)),
      'Was Dir ab Tag 1 monatlich aus der Tasche geht oder bleibt (heutiger Vertragszustand). Deine Mieten + Subvention − Annuität − Rücklage − Hausverwaltung − Mietverwaltung + Dein Steuervorteil. Positiv = Cashflow positiv für Dich (= Überschuss).', cls(_belTag0Disp(r))),
...
function renderStories(r) {
  ...
  const marktQuellenHinweis = (isPrice || hdPrice) ? `
    <div class="text-tertiary text-small" style="margin-top:8px;">
      ...
      ${marktQuelleLabel(marktSrc) ? ` — angezeigt: Marktwert ${marktQuelleLabel(marktSrc)}` : ''}
    </div>
  ` : '';
  ...
  const markteinkauf = (marktQm > 0) ? story('01 — Markteinkauf', _meHeadline, `
    <div class="story-grid">
      <table class="story-table">
        ...
        <tr><td>Marktwert${marktQuelleLabel(state.kalk._marktpreisQuelle) ? ' ' + marktQuelleLabel(state.kalk._marktpreisQuelle) : ''} / qm</td><td class="num">${Math.round(marktQm).toLocaleString('de-DE')} €/qm</td></tr>
...
function renderStoryPremium(r) {
  ...
  const selbsttragungPct = laufendeKostenMo > 0
    ? Math.min(_belTag0Disp(r) >= 0 ? 100 : 99, Math.round(einnahmenMo / laufendeKostenMo * 100))
    : 0;
      ...
      <div class="kalk-c-objekt-list">
        ...
        <div>
          ...
          <div class="kalk-c-objekt-row" title="${(() => {
            ...
            const srcText = marktQuelleLabel(src) ? ' · angezeigt: Marktwert ' + marktQuelleLabel(src) : '';
    ...
    <section class="kalk-c-section">
      <div class="kalk-c-section-head">
        <div class="kalk-c-left">
          ...
          <h2 class="kalk-c-section-title">${_belTag0Disp(r) >= 0 ? `Dein Überschuss ab Tag 1: +${fmtEurMo(_belTag0Disp(r))}.` : `Effektive Belastung ab Tag 1: ${fmtEurMo(_belTag0Disp(r))}.`}</h2>
        </div>
        <div class="kalk-c-right">
          ${_belTag0Disp(r) >= 0
            ? 'Die Wohnung trägt sich bereits ab Tag 1 vollständig selbst. Was bleibt, ist ein monatlicher Überschuss.'
            : (selbsttragungPct >= 95
              ? `Die Wohnung trägt sich zu rund ${selbsttragungPct} % selbst — die fehlenden ${100 - selbsttragungPct} % leistest Du als monatliche Eigenleistung von ${fmtEurMo(Math.abs(_belTag0Disp(r)))}, die mit jedem Jahr kleiner wird.`
      ...
      <div class="kalk-c-two-col">
        ...
        <div class="kalk-c-col-text">
          ...
          <p>${_belTag0Disp(r) >= 0
            ? `Die Wohnung trägt sich bereits ab Tag 1 vollständig selbst — Miete und Steuervorteil decken alle laufenden Kosten und liefern einen monatlichen Überschuss von ${fmtEurMo(_belTag0Disp(r))}.`
          ...
          ${_belTag0Disp(r) < 0 ? `<p>${crossoverSatz}: Die Wohnung beginnt, einen monatlichen Überschuss zu liefern, während Deine Annuität konstant bleibt.</p>` : ''}
...
function _rechnerBasis(d) {
  ...
  const istGarage = (t) => /garage|carport/i.test(String(t || ''));
  const garageKp  = details.reduce((s, x) => s + (istGarage(x.typ) ? (x.kaufpreis || 0) : 0), 0);
...
function _rechnerEnsureStyles() {
  ...
  s.textContent = `
    ...
    .rc-disclaimer{font-size:11.5px;font-style:italic;color:var(--text-tertiary);background:var(--bg-cream-subtle);border:1px dashed var(--border-strong);border-radius:10px;padding:11px 14px;margin-bottom:12px;line-height:1.55;}
...
function _rechnerRenderContent() {
  ...
  const lageParts = String((d.we && d.we.lage) || '').split(',').map(s => s.trim()).filter(Boolean);
  const adresse = lageParts.length >= 3 ? lageParts.slice(-2).join(', ') : ((d.we && d.we.lage) || '');
  const lageText = (d.we && d.we.lageText) || '';
  const etage = lageText.split(/\s+/)[0] || '–';
  const lageSeite = lageText.split(/\s+/).slice(1).join(' ') || '–';
  const stplNr = (t) => { const m = String(t || '').match(/StPl:\s*\d+,\s*([^,]+?)\s*,/); return m ? m[1].trim() : ''; };
  ...
  const spalteLinks = [
    sektion('Objektdaten', [
      ...
      (flaechenNrn ? zeile('Stellplatznummer', esc(flaechenNrn), { fix: true }) : ''),
...
function marktQuelleLabelRechner(quelle) {
  const q = String(quelle || '');
  if (q.includes('immoscout')) return 'ImmoScout24';
  if (q.includes('homeday')) return 'Homeday';
  return 'Marktdaten';
}
